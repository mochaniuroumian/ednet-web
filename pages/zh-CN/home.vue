  ...
  <section class="home">
    ...
    <section class="container">
      <section v-if="group3" class="product-block">
        ...
        <ul>
          <li
            v-for="item in group3.children"
            :key="item.id"
            @click="goNewsGroup(item.id,group3.type)"
          >
...
export default {
  ...
  async asyncData({ isDev, route, store, env, query, req, res, redirect, error }) {
    ...
    group1.items = group1.items.slice(0, 6)
    group2.items = group2.items.slice(0, 6)
    group3.items = group3.items.slice(0, 6)
